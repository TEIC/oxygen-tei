  ...
  <xsl:template name="schemaSpecWeave">
    ...
    <xsl:choose>
      <xsl:when test="$summaryDoc='true'">
        ...
        <xsl:for-each select="classSpec[@type='model' and @mode]  | classSpec[@type='atts' and @mode]  | macroSpec[@mode]  | elementSpec[@mode]">
	  ...
	  components</h2>
        <table>
          <xsl:for-each select="classSpec[@type='model' and not(@mode)]  | classSpec[@type='atts' and not(@mode)]  | macroSpec[not(@mode)]  | elementSpec[not(@mode)]">
            <xsl:sort select="@ident"/>
            <tr>
